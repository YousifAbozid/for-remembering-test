...
const resolvers = {
    Query: {
        ...
        allBooks: (root, args) => {
            if (!args.author && !args.genre) { 
              ...
              const booksByAuthorAndGenre = books.filter((b) => 
    ...
    Author: {
      bookCount: (root) => {
        const writtenBooks = books.filter(b => b.author === root.name)
        return writtenBooks.length
        }
    },
    Mutation: {
      addBook: (root, args) => {
        // if author doesn't exist add him to the authors list
        if (!(authors.find((author) => author.name === args.author ))) {
          const newAuthor = { name: args.author }
          authors = authors.concat(newAuthor)
          // console.log(authors)
        }
        // if the user exist add the book to the books list
        const newBook = {
          title: args.title,
          author: args.author,
