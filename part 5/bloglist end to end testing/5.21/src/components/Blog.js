    ...
    <div style={blogStyle}>
      <div id="blog" >
        <div className='short-details' style={hideWhenShowAll}>
          <p>{blog.title} {blog.author} <button id="full-details-button" onClick={() => setShowAll(!showAll)}>
          {showAll ? 'hide' : 'view' }
          </button>
          </p>
        </div>
        <div style={showWhenShowAll} >
          ...
          <p>{blog.author}</p>
          { createByUser(blog) && <button id="remove-blog" onClick={handleDelete}>remove</button> }
        </div>
      </div>
  </div>
)}
